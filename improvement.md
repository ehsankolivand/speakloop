...
- [x] **IMP-009 — Align the grammar prompt's "minimal span" rule with the coherence filter's 2-token floor**
  - Impact: Medium
  - Area: Correctness
  ...
  - Effort: Small
  - Resolution: Implemented prompt-side, but CORRECTED the mechanism: the entry's "one adjacent word / 'two childs'" is empirically insufficient — V2 also applies `MAX_UNKNOWN_FRACTION=0.25`, so a 2-token span with the 1 unknown error word is 50% unknown and still dropped (verified against the live filter). A single-word L2 error survives only when quoted inside a short PHRASE (~4 tokens → 1 unknown = 25%). Changed `grammar_analyzer.py`'s "minimal span" rule to ask for the broken part inside a short readable phrase (never a lone word), and added a worked example quoting "The childs are playing"/"they goed to school" (both verified to pass coherence). No test pins the prompt text (the ref test compares the `_SYSTEM_PROMPT` variable); the coherence `<2` floor stays pinned. Documented in `feedback/CLAUDE.md`. Test: `test_single_word_error_survives_when_quoted_as_a_phrase` (lone token AND 2-word span dropped; 4-word phrase survives). Verified: feedback suite 126 passed, full suite 878 passed (+1), ruff clean (the 2 B905/SIM103 findings pre-exist on HEAD, outside my diff).

- [x] **IMP-010 — Surface the reason when background feedback crashes in the drills path**
  - Impact: Medium
  - Area: Correctness
  ...
  - Effort: Small
  - Note: the code now lives in `_run_analysis_phase` (extracted in IMP-003), not the old inline `_bg_analyze`/join lines.
  - Resolution: `_bg_analyze` now records `holder["error"] = repr(e)`; after the join, when `outs is None`, the coordinator prints one yellow line (with a `speakloop resume` hint) AND threads the reason into `phase_c_error` via `pending_outs._replace(...)`. Degrade-to-resumable-pending is unchanged, and a normal drills run never sets `holder["error"]` → still byte-identical (byte-identical + drills-additive gates pass). Documented in `sessions/CLAUDE.md`. Test: `test_background_feedback_crash_surfaces_reason_instead_of_dropping_it` (monkeypatches `coordinator._analyze` to raise; asserts pending + `phase_c_error` carries the reason + the report + the yellow line show it). Verified: full suite 879 passed (+1), ruff clean.

- [x] **IMP-011 — Give the coverage/keypoints JSON calls the same bounded regenerate as grammar**
  - Impact: Medium
  - Area: Correctness
...
- [x] **IMP-014 — Repaint the debrief read-aloud in place via `rich.Live` instead of reprinting the whole view per section**
  - Impact: Medium
  - Area: UX
  ...
  - Effort: Medium
  - Resolution: `_read_aloud` now branches on `supports_live(console)`: on a real terminal it opens `renderer.live()` and `_on_section` calls `live.update(renderer.build(highlight_ref=..., progress_text=...), refresh=True)` — moving the highlight in place instead of re-emitting the whole composed view (which stacked N duplicate copies and scrolled). Non-terminals keep the per-section `print_static` fallback, so captured-output tests are unchanged; highlight rendering is identical (same `renderer.build`). `test_degradation.py` uses `force_terminal=True`, so `test_tts_failure` already exercises the Live path (on_section fires before synth); added `test_read_aloud_live_path_repaints_and_reaches_menu` for the full multi-section Live path. `live()`/`supports_live` are no longer dead code; documented in `debrief/CLAUDE.md`. Verified: debrief suite 41 passed, full suite 889 passed (+1), ruff clean (1 pre-existing UP037 on line 34, outside my edit).

- [x] **IMP-015 — Break up the 240-line `practice.run()` orchestrator**
  - Impact: Medium
  - Area: Structure
  ...
  - Effort: Medium
  - Resolution: Extracted `_provision_models(engine_choice, *, listen_only, console)` (base-phase ensure with abort-on-decline `typer.Exit(1)` + optional Phase-C degrade) and `_build_analysis(engine_choice, console) -> (grammar_analyzer, coach_runner, runners, analysis_parallel_safe)` from `practice.run()`; `run()` now reads as a short named sequence. Everything else (tts/play/key_reader setup, listen_only branch, mic check, ASR build, drills resolve, session/debrief loop) stays inline. Behavior verified EQUIVALENT by a fresh-context verifier subagent (line-by-line diff) + realized the "now unit-testable" benefit with `test_practice_helpers.py` (7 cases: base decline/failure abort, cloud base-only, local base+C, Phase-C decline/failure degrade, listen-only Phase-A). Documented in `cli/CLAUDE.md`. Note: overlaps the later IMP-017 (typed container), which will refactor `_build_analysis`'s internals. Verified: cli suite + help-isolation pass, full suite 896 passed (+7), ruff clean (2 findings at practice.py:111/142 pre-exist, outside my edit).

- [x] **IMP-016 — Consolidate the three blocking raw-key readers**
  - Impact: Medium
  - Area: Structure
  ...
  - Effort: Medium
  - Resolution: Added `sessions.keyboard.read_key_blocking(*, decode, line_parse, read_bytes=1, eof_value)` + `_cbreak_read_bytes(fd, read_bytes)`, centralizing the stdin-then-`/dev/tty` cbreak ladder + line-buffered fallback. Deleted `practice._cbreak_read` and `menu._cbreak_read_key`; `practice._read_key` (decode=`_decode_listen_key`, case-sensitive r/R, 1 byte, eof="") and `menu.read_key` (decode=`_decode_key`, arrows, 3 bytes, eof="quit") now route through it. Dropped the now-unused `import os` from both. No cycle / no engine load (help-isolation passes; debrief is function-local). Fresh-context verifier confirmed EQUIVALENT (the None-vs-empty-bytes tier fallthrough is preserved via `is not None`; two structural diffs proven unobservable). Updated all four divergence notes (root Trap 6, sessions/cli/debrief CLAUDE.md). Tests: `read_key_blocking` line-buffered + EOF paths, `_decode_listen_key` case-sensitivity, updated `test_menu.py` to patch `keyboard.os`. Verified: full suite 899 passed (+3), ruff clean (only the pre-existing UP042 on menu's DebriefChoice enum).

- [x] **IMP-017 — Replace the stringly-attached `.runners`/`.engine` on analyzer functions with a typed container**
  - Impact: Medium
  - Area: Structure
  ...
  - Resolution: Added frozen `practice.GrammarAnalysis(runner, runners, engine, coach)` (all fields REQUIRED → forgetting `engine` is a `TypeError` at construction) with a `parallel_safe` property; `_NO_ANALYSIS` = all-None sentinel for "no local model". All three `_build_*_grammar_analyzer` builders now return it (was: local bare callable / cloud+claude `(runner, coach)` tuples with `.runners`/`.engine` monkeypatched on). `_build_analysis` returns the dataclass; `run()` and `resume.py` read `analysis.runner`/`.coach`/`.runners`/`.parallel_safe`. Scope was larger than the entry billed (the builder return-type change touched ~7 test files that unpacked the tuples or monkeypatched `_build_grammar_analyzer`) — all updated mechanically. Documented in `cli/CLAUDE.md`. Tests: construction-requires-engine (TypeError), parallel_safe reads the engine, `_NO_ANALYSIS` is serial+runnerless. Verified: full suite 901 passed (+2), ruff clean, byte-identical/analysis-equivalence gates green.

- [x] **IMP-018 — Unify the duplicated device-loss/resample recovery ladder in playback**
  - Impact: Medium
  - Area: Structure
...
- [x] **IMP-020 — Add mypy on the pure-logic modules to the dev group (it finds real issues today)**
  - Impact: Medium
  - Area: Tooling
  ...
  - Effort: Medium
  - Resolution: Added `mypy>=1.11` to `[dependency-groups] dev` + a `[tool.mypy]` scoped (`follow_imports=silent`, `ignore_missing_imports=true`) to srs/store/coverage/metrics + `pronunciation/gop.py`+`drill_runner.py`. Fixed all 5 errors mypy found (still present after IMP-008): `srs/queue.py` bound `_parse_date(it.next_due)` once via walrus; `drill_runner._hear_first` `speak` param → `Callable | None` (it already handled None); `coverage/content_errors.py` annotated `item: dict[str, object]` (was inferred `dict[str,str]`, rejecting the int enrichment fields). `uv run mypy` is now GREEN (20 files) and wired into CI as a BLOCKING step (chose blocking over the entry's "non-blocking first" since it passes clean and the version is lockfile-pinned → deterministic; trivially relaxable). Documented in root `CLAUDE.md` Commands. Verified: mypy clean, affected unit suites (srs/coverage/drill_runner) + full suite 902 passed, ruff clean.

- [x] **IMP-021 — Wire up the unused `live_llm` marker: fix its filter/name and add the smoke test it implies**
  - Impact: Medium
  - Area: Testing
  ...
  - Effort: Medium
  - Resolution: Added `tests/live_llm_test.py` (`@pytest.mark.live_llm`, self-skips via `validator.validate(QWEN3_14B_4BIT).ok` + `importorskip("mlx_lm")`, mirrors `live_pron`): calls the real `make_sampler(temp,top_p,top_k,min_p)` / `make_logits_processors(repetition_penalty,repetition_context_size)` with the exact kwargs the wrapper+fakes assume, then runs a full `QwenEngine().generate(..., max_tokens=8)` end-to-end. Added `and not live_llm` to `addopts` and corrected the marker text (`Qwen3-8B`→`Qwen3-14B-4bit`, `mirrors live_asr`→`mirrors live_pron`, added the API-validation purpose). Documented in `.claude/rules/testing.md`. Verified: excluded from the default collection (0 collected), and `-m live_llm` actually RAN + PASSED on this model-equipped machine (loaded the real 14B, validated the mlx_lm API); default suite unchanged at 902 (now 4 deselected), ruff clean, context-budget green.

- [x] **IMP-022 — Unit-test `Wav2Vec2Scorer._read_audio` silence/short-clip gating with the committed silent WAV**
  - Impact: Medium
  - Area: Testing
...
- [x] **IMP-024 — Add a substitution (true-positive) case to the live pronunciation calibration oracle**
  - Impact: Medium
  - Area: Testing
  ...
  - Effort: Medium
  - Resolution: Added `test_a_deliberate_substitution_flags_the_target_phone` to `tests/live_pron_test.py` (`-m live_pron`, self-skips without models): for 5 minimal-pair substitutions (`rest`→west /w→ɹ/, `west`→vest /v→w/, `sin`→thin /θ→s/, `doze`→those /ð→d/, `seat`→sit /ɪ→iː/) it TTS-renders the WRONG word and scores it against the target word-drill's canonical, asserting the target phone flags. Used the clean word follow-ons (word-level canonicals) rather than the sentence bases for robust alignment; confusion words come from each drill's own `say_like` cue. EMPIRICALLY VERIFIED on real audio — ran `-m live_pron` (both models present): all 5 substitutions flagged; the full oracle (`2 passed`) now guards both over- AND under-flagging. Documented in `pronunciation/CLAUDE.md`. Excluded from the default suite (0 collected); default suite 911 passed, ruff clean.
