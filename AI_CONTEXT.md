...
10. **Build the grammar analyzer (Phase C, optional).** If `Qwen3-8B-4bit` validates as
...
| `src/speakloop/llm/` | LLM wrapper (Qwen3-8B 4-bit) behind a stable Protocol for Phase-C grammar feedback; **owns `mlx_lm`**. | `interface.py`, `qwen_engine.py` | `LLMEngine` Protocol (`generate(system, user, max_tokens=2048, temperature=0.7) -> str`), `LLMEngineError` | `installer` | `cli`, `feedback` |
...
| `src/speakloop/feedback/` | Session-report assembly (versioned frontmatter + Markdown body) plus the educational grammar/coherence analysis. | `frontmatter.py`, `markdown_writer.py`, `report_builder.py`, `grammar_analyzer.py`, `catalog.py`, `coherence.py`, `narrative.py` | `frontmatter.Session/dump/parse` (schema 1), `markdown_writer.write_atomic/next_available_path`, `report_builder.build(session)`, `grammar_analyzer.analyze(transcripts, llm)`, `catalog.get_catalog`, `narrative.build_narrative/select_top_priority`, `coherence` filter | `asr`, `config`, `llm` | `cli`, `debrief`, `sessions` |
...
| **TTS / ASR / LLM** | Text-to-speech (Kokoro), automatic speech recognition (Whisper default / Parakeet fallback), large language model (Qwen3-8B). Each behind a swappable Protocol. |
...
| A new **grammar pattern / catalog entry** | Extend the Persian-L1 catalog; keep the verbatim-substring guarantee for evidence quotes. | `feedback/catalog.py`, `feedback/grammar_analyzer.py` |
