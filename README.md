...
Local English interview-practice CLI for senior engineers. Runs Kokoro TTS, Whisper
ASR (Parakeet fallback), and Qwen3-8B LLM **fully offline** on Apple Silicon, walks you through a 4/3/2
...
Thirteen fine-grained modules under `src/speakloop/`, one responsibility each,
each with its own `CLAUDE.md`. Engine-specific imports live in exactly one file
per engine (`tts/kokoro_engine.py`, `asr/whisper_mlx_engine.py`, `asr/parakeet_engine.py`, `llm/qwen_engine.py`; Silero VAD is isolated in `asr/vad.py`).
