...
- **`doc/research_methodology.md`**: The authoritative reference for the practice methodology (shadowing, 4/3/2 task repetition, error-tagged feedback, L1-transfer patterns). The Functional Requirements covering fluency metrics and grammar-pattern detection (FR-012, FR-013) resolve their specifics against this document. The constitution (Principle X) requires this file to exist; at spec time it is referenced in the user's intent but is **not yet present in `doc/`** — it MUST be in place before the implementation plan for the feedback module is finalized.
