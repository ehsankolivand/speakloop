...
- **FR-003**: Each of the 19 modules under `src/speakloop/` (all currently have a CLAUDE.md) MUST end with either a rewritten CLAUDE.md holding only verified local invariants, extension points, and gotchas an agent cannot cheaply rediscover from the code, or no CLAUDE.md when it has no real local rules (delete-as-noise). The keep/delete decision is made per-file during the claim audit.
