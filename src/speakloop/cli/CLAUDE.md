...
- `deck` (018, `main.py` → thin `deck.py`) — rescue-lines deck trainer, OUTSIDE a session. Self-graded hear → say → see → self-mark over the learner's OWN corrected lines (derived from report grammar evidence via `linecards`) + bundled starter cards; reschedules each card on the shared SRS ladder (`linecards.advance_card` → `srs.schedule.advance`) and persists per-card state in the store `line_cards` section. Flags: `--limit N` (run cap, default `deck_daily_capacity`=20); `--export PATH` (write the whole deck as an Anki `{{c1::…}}` cloze file, then exit — no drilling, no models); `--ahead` (drill soonest-due when nothing is due). **TTS-only**: provisions Phase A only (no ASR, no scorer, no mic); writes NO report. Non-interactive → drilling skipped (self-mark needs a TTY); export still works. All engine imports function-local; `_is_interactive` is the test seam.
- `shadow` (018, `main.py` → thin `shadow.py`) — answer-shadowing trainer, OUTSIDE a session. Splits a chosen question's `ideal_answer` into sentences (`shadowing.split_sentences`, abbreviation-aware); per sentence TTS speaks it → learner repeats → resident ASR transcribes → deterministic OFFLINE feedback: content-word completeness (`shadowing.judge_completeness`) + pace/fillers (`metrics.compute_all`). Flags: `--question ID` (or interactive picker); `--limit N` (sentence cap); `--slow/--no-slow` (slower first read; `r` replays slower). Provisions Phase B (TTS + ASR) only — NO phoneme scorer, NO LLM; recording deleted after transcription; writes NO report (ephemeral). Non-interactive → skipped. All engine imports function-local; `_is_interactive` is the test seam.
...
**Pronunciation drills (016)**: `practice._resolve_pronunciation_drills(engine, console, *, drills_flag, input_fn)` runs ONCE before the session loop: reads `loop.yaml pronunciation_drills` (+ `--drills/--no-drills`), runs `pronunciation.assess_safety` (engine + live RAM), offers/declines (auto offers, on auto-runs when safe, off short-circuits before the gate), and on opt-in downloads the model via `installer.ensure_pronunciation_model` + builds a `coordinator.PronunciationDrills` bundle (else None). Unsafe → warn + skip unless an explicit interactive freeze-warned override — the prompt + `is_interactive` live in the shared `cli/gate_prompt.py` (`confirm_freeze_override` + `is_interactive`, IMP-031), re-exported by both `practice` and `pronounce` as their `_is_interactive` test seam. `local` engine is always unsafe (never silently loaded). All pronunciation imports are function-local → `--help` stays model-free. `doctor._pronunciation()` reports model presence (opt-in, never FAIL), the setting, and a gate estimate.

**Engine-aware provisioning (015)**: `practice._provision_models(engine_choice, *, listen_only, console)` (extracted from `run()` by IMP-015, unit-tested) ensures the required base phase (`"A"` listen-only / `"B"` full; decline/failure → `typer.Exit(1)`). Then, when `installer.engine_needs_local_llm(engine_choice, listen_only=...)`, it offers `ensure_models("C")` (the local Qwen) — declining/failing degrades to a recorded, resumable session (one notice, no exit); cloud engines never reach this. The feedback bundle build is `practice._build_analysis(engine_choice, console) -> GrammarAnalysis` — a frozen dataclass `GrammarAnalysis(runner, runners, engine, coach)` with a `parallel_safe` property (IMP-017; the three `_build_*_grammar_analyzer` builders return it, `_NO_ANALYSIS` = all-None when no local model). All fields are REQUIRED so a builder forgetting `engine` fails at construction instead of silently going serial (replaces the old bolted-on `.engine`/`.runners` attributes). `run()`/`resume.py` read typed fields (`analysis.runner`/`.coach`/`.runners`/`.parallel_safe`). `setup.py` reuses the same predicate (base `"B"` always; `"C"` only for local). Default engine is set once via `speakloop setup` (`config.loop_config.save_engine`).
...
- `deck.py` (018) — `run(*, limit, export_path, ahead, tts_engine, play_fn, reports_dir, store_path,
  starter_cards, today, input_fn, console)`: derive+merge cards (`linecards`) → export (whole-deck
  Anki cloze, no models) OR self-graded drill (provision Phase A → `speak` closure → hear/say/see/
  self-mark → `linecards.advance_card` → persist store `line_cards`). TTS-only, no report; `_MARKS`
  maps 1/2/3/4 → poor/fair/good/strong; `_is_interactive` is the test seam.
- `shadow.py` (018) — `run(*, question_id, limit, slow, tts_engine, play_fn, record_fn,
  transcribe_fn, key_reader, qa_file, scratch_dir, input_fn, console)`: resolve+load Q&A → pick
  question → `shadowing.split_sentences` → provision Phase B (TTS+ASR, NO scorer/LLM) → per sentence
  hear (`speak`, `--slow`/`r` slower) → record (`coordinator._record_stage`) → `transcribe_fn` →
  delete wav → `_show_feedback` (`shadowing.judge_completeness` + `metrics.compute_all`). Ephemeral;
  no report/store write. `transcribe_fn` is the ASR test seam; `_is_interactive` the TTY seam.
...
- `gate_prompt.py` (IMP-031) — shared pronunciation-gate UX: `is_interactive()` + `confirm_freeze_override(console, *, input_fn, interactive)`. One copy of the freeze-warned override prompt for `practice` + `pronounce`.
